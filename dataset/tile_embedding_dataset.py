    ...
    def __init__(self, excel_path="../data/merge.xlsx", embedding_dir="../data/tile224embedding"):
        ...
        for index, row in df.iterrows():
            ...
            for file_name in matching_files:
                ...
                self.embedding_list.append((embedding_path, pfs, 1 - int(row["最后随访状态"])))

    def __len__(self):
        return len(self.embedding_list)

    def __getitem__(self, index):
        embedding_path, pfs, dead = self.embedding_list[index]
        embedding = torch.load(embedding_path)
        output = embedding[:, :1536]
        position = embedding[:, 1536:]
        return output.float(), position.float(), torch.tensor(pfs).float(), torch.tensor(dead).float()


def collate_fn(batch):
    embeddings, position, pfs, dead = zip(*batch)
    ...
    return padded_embeddings, position, pfs, dead
